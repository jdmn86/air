...
public class US6_AT2 {
    ...
    public void dashboardActivityTest() {
        ...
        try {
            searchTextViewApp.setText("Peniche");
        } catch (UiObjectNotFoundException e) {
                fail("App search textView not found");

        }
        UiObject listItemToClick = mDevice.findObject(new UiSelector().text("Peniche").instance(1));
        try {
        ...
        appCompatEditText = onView(
                allOf(withId(R.id.place_autocomplete_search_input),
                        childAtPosition(
                                ...
                                1),
                        isDisplayed()));
        appCompatEditText.check(matches(withText("Peniche")));
        ...
        try {
            assertEquals(marker.getContentDescription().trim(),"Peniche, Portugal.");
        } catch (UiObjectNotFoundException e) {
